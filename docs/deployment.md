...
- **数据库中保存着渠道密钥与商户密钥的明文**（网关需要用它们向上游签名）。新建的数据目录为 `0700`、
  数据库文件为 `0600`，同机其他用户无法读取。若从旧版本升级，手动收紧一次：
  ```bash
  chmod 700 data && chmod 600 data/epay.db*
  ```
  备份文件同样属于密钥级资产，请放在受限目录并考虑加密。
- 配置文件中的密钥建议通过 `${ENV_NAME}` 从 `.env` 读取，并 `chmod 600 .env`；仓库已忽略
  `config.yaml`、`.env`、`Caddyfile`、`data/`、`certs/`，不会被误提交。
